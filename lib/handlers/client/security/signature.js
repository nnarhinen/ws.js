...
console.log("Using nnarhinen-fork of ws.js")

exports.Signature = Signature

function Signature(signingToken, options) {
	if (!signingToken)
	...
	if (options && options.signOptions) this.signOptions = options.signOptions
}
...
Signature.prototype.applyMe = function(doc, security) {
	...
	this.signature.computeSignature(doc.toString(), this.signOptions)
...
function WSSKeyInfo(signingToken) {
  ...
  this.getKeyInfo = function(key, prefix) {
    ...
    return `<${prefix}SecurityTokenReference>` +
           `<${prefix}Reference URI="#${signingToken.getId()}" ` +
